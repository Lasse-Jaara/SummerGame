...
def draw_window(mouse_pos, left_click, Hp_text, Round_text):
    ...
    level_multiplier = 20
    # WIDTH = 900
    xp_bar_rect = pygame.Rect(0, 0, WIDTH-WIDTH+round((xp_collected*level_multiplier)), 30)
    pygame.draw.rect(window, (0, 0, 0), xp_bar_rect)  # Fills the rect

    xp_bar_border_rect = pygame.Rect(0, 0, WIDTH, 30)
    pygame.draw.rect(window, (0, 0, 0), xp_bar_border_rect, 1)
    ...
    for orb in xp_list[:]:
        if orb.pygame_rect.collidepoint(mouse_pos):
            orb.sound()
            xp_list.remove(orb)
            xp_collected += 1
    for orb in xp_list:
...
def main():
    ...
    while running:
        ...
        draw_window(mouse_pos, left_click, Hp_text, Round_text)
        if castle_hp <= 0:
            break
    pygame.quit()
